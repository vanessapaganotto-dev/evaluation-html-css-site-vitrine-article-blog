🇬🇧 Version anglaise disponible : [README_EN.md](README_EN.md)


# Le Globule - Site Web Responsive HTML/CSS

---

## Description

  
Site web responsive en HTML et CSS présentant une structure classique avec un menu principal à sous-menus riches, un contenu principal en deux colonnes, et un pied de page simple.  
Le design utilise des listes de navigation, des figures avec images et légendes, et une organisation sémantique avec header, main, aside et footer.

---

## Fonctionnalités 

- Menu principal avec sous-menus en grille / Main menu with grid-based dropdown submenus  
- Images avec attributs alt pour accessibilité / Images with alt attributes for accessibility  
- Contenu principal organisé en colonnes / Main content organized in columns  
- Utilisation d'éléments HTML5 sémantiques / Use of semantic HTML5 elements  
- Mise en page responsive (via CSS externe) / Responsive layout (via external CSS)  

---

## Usage

- Ouvrir le fichier `index.html` dans un navigateur moderne.  
- Naviguer via le menu principal et ses sous-menus.  
- Lire les articles dans la colonne principale et les sections complémentaires dans l'aside.  
- Footer avec liens de navigation additionnels.

---

## Technologies utilisées 

- HTML5  
- CSS3 (liés dans les fichiers `../css/default.css` et `css/style.css`)

---

## Auteur 

Vanessa PAGANOTTO

---

## Licence

Ce projet est sous licence MIT.
